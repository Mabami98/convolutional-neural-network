...
// random 4d tensor used for conv filters
static Tensor4D random_tensor_4d(int F, int C, int H, int W, float mean=0.0f, float stddev=0.01f) {
    random_device rd;
    mt19937 gen(rd());
    normal_distribution<float> dist(mean, stddev);

    Tensor4D tensor(F, Tensor3D(C, vector<vector<float>>(H, vector<float>(W))));
    for (int f = 0; f < F; ++f)
        for (int c = 0; c < C; ++c)
            for (int i = 0; i < H; ++i)
                for (int j = 0; j < W; ++j)
...
// random weight matrix for fully connected layer
...
// flatten conv output into 1d vector
static vector<float> flatten(const Tensor3D& tensor) {
    vector<float> flat;
...
// relu activation done directly in the tensor
...
static Matrix softmax(const Matrix& scores) {
    ...
    for (int i = 0; i < batch_size; ++i) {
        ...
        float sum = 0.0f;
        for (int c = 0; c < num_classes; ++c) {
            probs[c][i] = exp(scores[c][i] - max_val); // subtract max to avoid overflow
            sum += probs[c][i];
        }
        for (int c = 0; c < num_classes; ++c)
            probs[c][i] /= sum;
...
// perform one 2d convolution at location (i_start, j_start)
...
Matrix CNN::forward(const Tensor3D& input) {
    ...
    relu_inplace(last_conv_out); // non-linearity
    ...
    return softmax(scores); // return probabilities
}
...
void CNN::backward(const Matrix& probs, const Matrix& labels) {
    ...
    // gradient through relu and conv output
...
void CNN::update_weights(float lr) {
    // update fully connected layer params
    for (int c = 0; c < num_classes; ++c) {
        b[c] -= lr * db[c];
        db[c] = 0.0f;
